...
# v1.1 修改于20210508 添加了干员幽灵鲨
# v1.2 修改于20210510 奖励干员从非可用职业中选择
# v1.3 修改于20210517 调整了爆率，以及函数化（貌似不好用，先看看吧）
import random
...
def roll_1_operator(pick_type,operator_dict):
    ban_operator = []
    if '近卫' in pick_type:
        # 近卫特殊规则：只有两个职业
        pick = []
...
ban_operator = roll_1_operator(pick_type,operator_dict)

if random.randint(0,2) == 0:
    a = list(operator_dict.keys())
    for a1 in ban_operator:
        a.remove(a1)
    lucky = random.sample(a, 1)
    while(operator_dict[lucky[0]] in pick_type):
        lucky = random.sample(a, 1)
    
    print('额外赠送的干员为：',lucky[0])
print()
print('被ban的干员为：',", ".join(ban_operator))
print()
print('可用的职业为：',", ".join(pick_type))
print()
print('--钢铁男儿左手窝窝屎右手砍口垒 群作业系统v1.3--')
print('--p.s.33%几率出稀有奖励干员--')
