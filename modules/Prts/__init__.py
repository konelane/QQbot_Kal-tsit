...
class Prts:
    ...
    def __MessageGet(self):
        ...
        kk = str(soup.find_all("textarea")[0]).split('==')#找到每个网页内容中的文字取出来
        opt_dict = dict()
    ...
    def __LogisticsSkillSearch(self):
        ...
        soup = bs(page.content, 'html.parser')  #解析网页
        # print(soup)
        kk = str(soup.find_all("textarea")[0]).split('技能名=')[1::] #找到每个网页内容中的文字取出来
    ...
    def __MsgJsonGet(self) -> dict:
        '''获取干员dict'''
        with open(os.path.dirname(__file__) + './gamedata/excel/character_table.json','r',encoding='utf8')as fp:
            json_data = json.load(fp)
        return json_data

    def __operatorChecker(self, operator_name):
        '''获取干员key'''
        exists_flag = 0
        operator_key = ''
        json_data = self.__MsgJsonGet()
        for key, val in json_data.items():
            if json_data[key]['name'] == operator_name:
                exists_flag = 1
                operator_key = key
                break
    ...
    def SkillLevel(self):
        '''获取干员skill'''
        with open(os.path.dirname(__file__) + './gamedata/excel/skill_table.json','r',encoding='utf8')as fp:
            skill_json_data = json.load(fp) # skill文件

        json_data = self.__MsgJsonGet() # 角色信息文件
        outtext = ''
        ifExist, opt_key = self.__operatorChecker(self.name) # 获取key
        if ifExist == 1:

            for skill in json_data[opt_key]['skills']:
                skill_id = skill['skillId']
                skill_name = skill_json_data[skill_id]['levels'][0]['name']
                skill_level_list = skill_json_data[skill_id]['levels']
                
                outtext += skill_name
                for level in skill_level_list:
                    blackboard = {}
                    for dict_val in level['blackboard']:
                        new_dict = {dict_val['key']: dict_val['value']}
                        blackboard.update(new_dict)
                        
                    desc = level['description']
                    for key, val in blackboard.items():
                        desc = desc.replace(str(key), str(val))
                    outtext += '\n' + desc 
                outtext += '\n***************\n'
    ...
    def PotentialAndTalent(self):
        ...
        if ifExist == 1:
            # 潜能
            outtext += '潜能\n'
            potential_list = json_data[opt_key]['potentialRanks']
            i = 0
            for poten in potential_list:
                i += 1
                msgtext = f'潜能{str(i)}:'+poten['description']
                outtext += msgtext + '\n'
            
            # 天赋
            outtext += '\n*************\n天赋'
            talent_list = json_data[opt_key]['talents']
            for i in range(len(talent_list)):
                for levelMsg in talent_list[i]['candidates']:
                    need_phase = str(levelMsg['unlockCondition']['phase'])
                    need_potential = str(levelMsg['requiredPotentialRank'])
                    msgtext = levelMsg['name'] + f'(精英{need_phase}潜能{need_potential}):' + levelMsg['description']
                    outtext += '\n' + msgtext 
...
@channel.use(
    ListenerSchema(
        ...
        inline_dispatchers=[Twilight([RegexMatch(r'\#prts.*').flags(re.X)])],
...
async def Prts_app(
    ...
    pass    
    """prts查询功能"""
    prts_mod = ['技能专精','晋升材料','属性','后勤','生日','今日生日','模组','技能', '潜能','天赋']
    exception_squad = ['凯尔希']
    ...
    if text_list[2] not in prts_mod:
        ...
        if text_list[2] == '技能':
            out_list = prts_1.SkillLevel()
        if text_list[2] in ['潜能','天赋']:
            out_list = prts_1.PotentialAndTalent()
