...
class GameDoctor:
    """博士状态"""
    operator_dict_resources: dict = SaverLoader.jsonReader(SaverLoader.operatorsDir)
    item_dict_resources: dict = SaverLoader.jsonReader(SaverLoader.itemDir)

    def __init__(self, doctor_condition=None) -> None:
        if doctor_condition is None:
            self.hp = HP_INIT
            self.coc5 = {
                "str": 60,  # 力量
                "dex": 60,  # 敏捷
            ...
            self.hp = doctor_condition["hp"]
            self.coc5 = doctor_condition["coc5"]
            self.bag = doctor_condition["bag"]
            self.bag_dict = doctor_condition["bag_dict"]
            self.friends = doctor_condition["friends"]
            self.friends_dict = doctor_condition["friends_dict"]
            self.kaltsit_anger = doctor_condition["kaltsit_anger"]
    ...
    def setFriend(self, opt_name, opt_dict=None, opt_type="new"):
        ...
        if opt_type == 'new':
            ...
            self.friends_dict[opt_name] = self.operator_dict_resources["operator"][opt_name]
    ...
    def setBag(self, item_name, item_dict=None, item_type="new"):
        ...
        if item_type == 'new':
            ...
            self.friends_dict[item_name] = self.item_dict_resources["item"][item_name]
