...
class Move:
    ...
    def __init__(self, msg_dict) -> None:
        ...
        self.save_new_dir = "resources/saves/" + str(self.group_id) + "_" + str(
            self.save_length) + '.json'  # 共有x个存档，则下一个存档为x号
        self.read_save_dir = "resources/saves/" + str(self.group_id) + "_" + str(
            max(0, self.save_length - 1)) + '.json'  # 默认最后一个存档
    ...
    def toStringOutText(self):
        ...
        outtext += '\n[故事是否结束]' + (
            '是, 结局编号为' + save_dict["endding_id"] if save_dict["is_end"] == 1 else '否')
    ...
    def nextMoveAction(self, node_obj: Union[ChoiceNode, DiceNode]) -> MoveAction:
        """核心方法 读取并进行下一步动作
        理论上应执行至需要Input的步骤停止
        @:returns MoveAction
        """
        ...
        while not node_obj.isNodeDone():
            ...
            if next_move_order == 'choice':
                ...
                outtext += '===============\n【check】\n未达到check条件。'
                break
        # 首先检查节点是否有rqmt未达成 若有卡点 则返回【当前节点】，重新执行。
        print('[nextMoveAction]next_move_order: ', next_move_order)
        if next_move_order in ['rqmt', 'dice', 'choice']:
            print('[nextMoveAction] set move_action')
            # outtext_fail, next_node_name, text_type = node_obj.rqmtFailNode() # 废弃

            move_action.set_outtext(outtext)
            move_action.set_extra_flag(True)
            move_action.set_node_name(node_obj.node_name)  # 停留在当前节点

            move_action.set_extra_msg_type(next_move_order)
            return move_action
        ...
        if node_obj.isRewardExist():
            ...
            if reward_type == 'endding':
                ...
                # 若为结局节点 - 单独执行一下结局文本（看看怎么设计 TODO

        self.checkIsEndding(node_obj)  # 检查并更改游戏结局情况
        self.updateNodeSave(node_obj)

        move_action.set_outtext(outtext)
        move_action.set_extra_flag(False)
        move_action.set_extra_msg_type('auto')
        return move_action
    ...
    def choiceMove(self, input_choice, node_obj: ChoiceNode):
        ...
        if node_obj.nextMove(self.read_save) == 'choice':
            ...
            move_action: MoveAction = self.nextMoveAction(node_obj)
            if not move_action.if_extra_flag():
                return move_action.get_outtext(), out_list[0], out_list[1]
            elif move_action.if_extra_flag():
                if move_action.get_extra_msg_type() == 'rqmt':
                    # 将传来的字典进行解析后传回: outtext, node_xxx, "node_name"
                    return move_action.get_outtext(), move_action.get_node_name(), 'node_name'
                else:
                    print('[move_action]均不符合-choice')
                    return move_action.get_outtext(), out_list[0], out_list[1]
    ...
    def diceMove(self, node_obj: DiceNode):
        ...
        if node_obj.nextMove(self.read_save) in ['dice']:
            outtext, result_text, result_type = node_obj.inputDice(self.read_save)
            node_obj.updateNodePosition()
            move_action: MoveAction = self.nextMoveAction(node_obj)
            outtext += move_action.get_outtext()
            # print('[diceMove - check1]', move_action.get_node_name(), '\n', move_action.get_outtext(), '\n', move_action.get_extra_msg_type(), '\n', move_action.if_extra_flag(), '\n***********')
            if not move_action.if_extra_flag():
                return outtext, result_text, result_type
            elif move_action.if_extra_flag():
                if move_action.get_extra_msg_type() == 'rqmt':
                    # 将传来的字典进行解析后传回: outtext, node_xxx, "node_name"
                    return outtext, move_action.get_node_name(), 'node_name'
                else:
                    print('[move_action]均不符合-dice')
                    return outtext, result_text, result_type
        else:
            print('[dice ERROR!] This node is not diceNode!')
            return '', None, 'None'
    ...
    def diceCheatMove(self, node_obj: DiceNode, point_cheat):
        ...
        if node_obj.nextMove(self.read_save) == 'dice':
            ...
            print('[zuobitouzi]', outtext, result_text, result_type)
            node_obj.updateNodePosition()
            move_action: MoveAction = self.nextMoveAction(node_obj)
            outtext += move_action.get_outtext()
            if not move_action.if_extra_flag():
                ...
                if move_action.get_extra_msg_type() == 'rqmt':
                    ...
                    print('[move_action]均不符合-cheat')
                    return outtext, result_text, result_type
        else:
            print('[Cheat dice ERROR]')
            return '', None, 'None'
    ...
    def ifPlayerInMemberList(self):
        ...
        return self.msg_dict["id"] in self.read_save.member_list or int(
            self.msg_dict["id"]) in self.read_save.member_list
