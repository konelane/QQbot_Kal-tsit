...
class kalrogueService:
    ...
    def run(self, order_status):
        ...
        if order_status == '0':
            ...
            # 执行默认步骤
            # TODO 这里要改下 现在所有节点使用go会直接跳过，进入next 包括dice节点和choice节点，choice节点会直接进入 next_node_id
            outtext = ''
            if current_node.node_type == 'endding':
                # 已结局
                move_action: MoveAction = start_obj.move_obj.nextMoveAction(current_node)
                return move_action.get_outtext()
            elif current_node.node_type == 'autotext':
                move_action: MoveAction = start_obj.move_obj.nextMoveAction(current_node)
                ...
                if not move_action.if_extra_flag():
                    ...
                    if move_action.get_extra_msg_type() == 'rqmt':
                        ...
                        start_obj.moveToNewNode(move_action.get_node_name())
            else:
                # 若不是自动文本节点
                move_action: MoveAction = start_obj.move_obj.nextMoveAction(current_node)
                outtext += move_action.get_outtext()
                if not move_action.if_extra_flag():
                    # 自动文档直接进入下一节点
                    print('[显示下一节点]', current_node.next_node_id)
                    start_obj.moveToNewNode(current_node.next_node_id)
                elif move_action.if_extra_flag():
                    print('[显示下一节点 - 有额外信息]', current_node.next_node_id)
                    if move_action.get_extra_msg_type() == 'rqmt':
                        # 将传来的字典进行解析后传回: outtext, node_xxx, "node_name"
                        start_obj.moveToNewNode(move_action.get_node_name())
            ...
            if len(self.msg_info['text_split']) >= 3:
                ...
                if not credit_result:
                    return outtext_credit  # 若未能通过信用检查，则返回

                outtext, choice_result, choice_result_type = start_obj.move_obj.choiceMove(
                    self.msg_info['text_split'][2], current_node)
            ...
            if not move_action.if_extra_flag():
                # 自动文档直接进入下一节点
                print('[dice go 显示下一节点]', current_node.next_node_id)
                start_obj.moveToNewNode(current_node.next_node_id)
            elif move_action.if_extra_flag():
                print('[dice go 显示下一节点 - 有额外信息]', current_node.next_node_id)
                if move_action.get_extra_msg_type() == 'dice':

                    # 传入骰子
                    outtext2, dice_result, dice_result_type = start_obj.move_obj.diceMove(current_node)
